...
int main()
{
	...
	struct sockaddr_in server_addr; // configuring some information about the server socket
	...
	hints.ai_protocol = 0;					 // automatically choose protocol type (wut dat mean), guides don't even have this set explicitly so

	// TODO - Haven't tested failure paths here
	int res;
	if ((res = create_server_socket(&hints, &server_fd, PORT, CONN_BACKLOG)) >= 1)
	{
...
/*
	using AI_PASSIVE means we don't have to hardcode the address during bind() call.
	Since i'm hardcoding localhost, i'm using numeric host, but ordinarily
	a server would want to use AI_PASSIVE
*/
...
struct sockaddr {
		unsigned short    sa_family;    // address family, AF_xxx (AF_INET | AF_INET6) for IPv4 & IPv6 in this context
		char              sa_data[14];  // 14 bytes of protocol address. Contains destination address & port number for the socket. Not large enough to accommodate a larger address like IPv6.
}; */
